...
from schemas.pedidos import ClienteResponse
...
@router.get("/clientes-cadastrados")
async def clientes_cadastados(db: Session = Depends(get_db), user_current: dict = Depends(get_current_user)):
    db_clientes = db.query(ClientesModel).filter(ClientesModel.estabelecimento_id == user_current["estabelecimento_id"]).all()
